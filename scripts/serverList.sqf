...
OO_TRACE_DECL(JB_SL_S_ListCreate) =
{
	...
	if (isNil "_list") then
	{
		...
		{
			[_list, _x select 0, _x select 1] call JB_SL_S_SetItemValue;
		} forEach _items;
...
OO_TRACE_DECL(JB_SL_S_GetItemValue) =
{
...
OO_TRACE_DECL(JB_SL_S_SetItemValue) =
{
...
OO_TRACE_DECL(JB_SL_S_ListSetItem) =
{
	...
	private _item = [_list, _itemName, _itemValue] call JB_SL_S_SetItemValue;
...
OO_TRACE_DECL(JB_SL_S_ListSetItemMember) =
{
	...
	private _item = [_list, _itemName] call JB_SL_S_GetItemValue;
